...
public class MainView extends AppCompatActivity implements IMainContract.View {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menuItemRemoveOutdated:
                item.setChecked(!item.isChecked());
                presenter.onIgnoreOutdatedChargers(item.isChecked());
    ...
    public void init() {
        ...
        findViewById(R.id.chipTodos).setTag(null);
        ...
        Stream.of(R.id.chipTodos, R.id.chipIberdrola, R.id.chipIonity, R.id.chipZunder)
                .forEach(i -> {
    ...
    private void onFilterClicked(Chip chip) {
        presenter.onOperatorFilterClicked((EOperator)chip.getTag());
    }

    private void onSortingClicked(Chip chip) {
        ESorting sorting = (ESorting) chip.getTag();
        switch (sorting) {
            case COST:
                presenter.onCostSortingClicked();
                break;
            case POWER:
                presenter.onPowerSortingClicked();
                break;
            case DISTANCE:
                presenter.onDistanceSortingClicked();
                break;
        }
    }
